...
public class IcebergRESTCatalog implements IcebergCatalog {
    ...
    //
    // Two shapes must be recognised:
    //   * Polaris-style servers return HTTP 401 -> NotAuthorizedException.
    //   * Gravitino returns the token-expired error as a generic RESTException (HTTP 419,
    //     unmapped by the Iceberg client): "Unable to process: Authentication token is expired".
    // Both mean the same thing, so treat either as recoverable and rebuild the session.
